 ...
 */
function SpinTwoThree(element, classes, slotsDelay, slotsSpinTime, usePx) {
  'use strict';
...
*/
  ...
  function Slot(slices, index, slotDelay) {
    ...
    function _spin() {
      ...
      spinCount[index] = 0;
      // console.log(index, spinCount[index]);
      ...
      if (_isSpinning === false || _override === true) {
        ...
        speedIncrement = 1;
        maxSpeed = 30;
        // console.log('speed', speedIncrement, maxSpeed);
        // jjailbird modified ----------------------------
    ...
    function PlaySelection(nStart, nEnd)
    {
      //audioRing = $get('track');
      ...
      nEndTimeX 		= nEnd;
      /*
      if( audioRing.readyState != 4 )
        audioRing.load();
      */
      audioRing.currentTime = nBeginTimeX;
      audioRing.play();
      if( myTimer != -1 )
        clearInterval( myTimer );
      myTimer = setInterval( function(){ Timer1() }, 1 );
    }
    
    function Timer1()
    {
      if( audioRing.currentTime >= nEndTimeX  )
      {
          audioRing.pause();
          if( audioRing.currentTime + 1 >= audioRing.duration )
    ...
    function _rollSlot() {
      ...
      spinCount[index] += 1;
      // console.log(audioRing.readyState);
      if (spinCount[index] % 6 == 0)
        PlaySelection(0.2, 0.3);
      // if(audioRing.readyState == 1) {
        //;
        // setTimeout(function() {audioRing.play()}, 1);
      // }
      

      // console.log('animStatus',spinCount[index],animStatus, slicesArray[1].dataset.value);
      //
      if (delta !== 0) {
        if (isVertical) {
          ...
          // console.log('delta', delta, slot.style.top);
        }
      }
      //
      ...
      if (Math.abs(delta) >= Math.abs(maxSpeed)) {
        ...
        // slot is stopping
        if (slicesArray[1].dataset.value == SlotNumbers[index])
          animStatus = -1;
      }
      ------------------------------------------------------------------------------*/


      /* Control Speed by jjaiblird ----------------------------------------*/
      if (animStatus !== -1 && _isSpinning !== false) {
        // slot is spinning
        var cb = _rollSlot.bind(this);
        requestAnimFrame(cb);
      ...
      else if (spinCount[index] > 100 & slicesArray[1].dataset.value == SlotNumbers[index])
        animStatus = -1;
      else if ((Math.floor(delta) <= speedIncrement && animStatus === 1) || (Math.floor(delta) <= speedIncrement && animStatus === 2)) {
        // slot is stopping
        animStatus = -1;
    ...
    function _gotoNearest() {
      ...
      if (isVertical) {
        ...
        // console.log('_gotoNearest',index,l,slicesArray[1].dataset.value == SlotNumbers[index]);
        TweenLite.to(slot, 0.35, {
          top: l,
          ease: Cubic.easeInOut,
          onComplete: cb
...
*/
  ...
  function _setSlotNumbers(numbers) {
    SlotNumbers = numbers;
  }
  ...
  return {
    ...
    setSlotNumbers: _setSlotNumbers,
